...
function getBaseName(fileName, fallback) {
...
export default function App() {
  ...
  const [pdfArticle, setPdfArticle] = useState('');
  ...
  const article = articleByMode[inputMode] ?? '';

  const improvedDownload = useMemo(() => {
    if (!improvedArticle) {
      return null;
    }
    ...
    if (inputMode === 'html') {
      return {
        content: injectText(htmlSource, improvedArticle),
        fileName: `${getBaseName(htmlFileName, 'improved-article')}-improved.html`,
    ...
    if (inputMode === 'pdf') {
      return {
        ...
        fileName: `${getBaseName(pdfFileName, 'improved-article')}-improved.txt`,
  ...
  function handleTextArticleChange(nextArticle) {
    setTextArticle(nextArticle);
    resetResultForNewInput();
  }

  function handleHtmlLoaded({ fileName, htmlString, extractedText }) {
    setHtmlFileName(fileName);
    setHtmlSource(htmlString);
    setHtmlArticle(extractedText);
    resetResultForNewInput();
    setError('');
    setNotice('HTMLファイルを読み込み、body内テキストを抽出しました。');
  }

  function handlePdfLoaded({ fileName, extractedText }) {
    setPdfFileName(fileName);
    setPdfArticle(extractedText);
    resetResultForNewInput();
    setError('');
    setNotice('PDFファイルからテキストを抽出しました。');
  }
  ...
  async function runScoring(targetArticle = article) {
    if (!targetArticle.trim()) {
      setError('採点する記事を入力またはアップロードしてください。');
      return;
    }
  ...
  function handleDownloadImproved() {
    ...
    downloadFile(improvedDownload.content, improvedDownload.fileName, improvedDownload.mimeType);
    setNotice(`${improvedDownload.fileName} をダウンロードしました。`);
  }

  function handleUseImproved() {
    if (inputMode === 'html') {
      setHtmlArticle(improvedArticle);
      setHtmlSource(injectText(htmlSource, improvedArticle));
    } else if (inputMode === 'pdf') {
      setPdfArticle(improvedArticle);
    } else {
      setTextArticle(improvedArticle);
    }
    ...
    <main className="min-h-screen px-4 py-6 text-slate-100 md:px-8 md:py-10">
      ...
      <div className="mx-auto max-w-7xl space-y-8">
        <header className="rounded-[2rem] border border-slate-700 bg-[#1e293b]/90 p-6 shadow-2xl shadow-slate-950/30 md:p-9">
          ...
          <div className="grid gap-5 lg:grid-cols-[1.4fr_0.6fr] lg:items-end">
            ...
            <div className="rounded-3xl bg-slate-950/70 p-5 text-sm leading-7 text-slate-300">
              ...
              <p>PDF入力はブラウザ上でpdf.js CDNを読み込みます。</p>
            </div>
          </div>
        </header>
        ...
        <ArticleInput
          ...
          onArticleChange={handleTextArticleChange}
