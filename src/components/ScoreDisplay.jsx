...
function getSalesBanner(total) {
  if (total >= 110) {
    return { message: '🔥 バズり候補！今すぐ公開してください', className: 'border-red-300/40 bg-red-500/25 text-red-100' };
  }

  if (total >= 100) {
    return { message: '⭐ 売れやすい記事です！公開推奨', className: 'border-emerald-300/40 bg-emerald-500/25 text-emerald-100' };
  }

  if (total >= 85) {
    return { message: '📈 もう少し！自動修正で100点超えを狙いましょう', className: 'border-yellow-300/40 bg-yellow-500/25 text-yellow-100' };
  }

  if (total >= 70) {
    return { message: '✏️ 改善が必要です。自動修正を試してください', className: 'border-orange-300/40 bg-orange-500/25 text-orange-100' };
  }

  return { message: '⚠️ 大幅な見直しが必要です', className: 'border-slate-400/40 bg-slate-500/25 text-slate-100' };
}
...
export default function ScoreDisplay({ result }) {
  ...
  const salesBanner = getSalesBanner(total);
    ...
    <section className="rounded-3xl border border-slate-700 bg-[#1e293b]/95 p-5 shadow-2xl shadow-slate-950/30 md:p-7">
      <div className="mb-6 flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        ...
        <div className="rounded-3xl border border-cyan-400/30 bg-slate-950/80 px-7 py-5 text-center">
          ...
          <p className="text-sm text-slate-500">/ 120</p>
          <p className="mt-2 text-xs text-slate-400">満点：120点 ｜ 売れる目安：100点以上</p>
        </div>
      </div>

      <div className={`mb-6 rounded-2xl border p-4 text-center text-lg font-black ${salesBanner.className}`}>
        {salesBanner.message}
      </div>
